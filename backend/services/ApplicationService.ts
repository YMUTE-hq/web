...
import { UserProfile, getErrorMessage } from "@/types";
import { User } from "@supabase/supabase-js";

export class ApplicationService {
  static async getApplications(user: User, profile: Partial<UserProfile>, jobId?: string) {
    try {
      ...
      throw new Error(`Failed to fetch applications: ${getErrorMessage(e)}`);
    }
  }

  static async submitApplication(user: User, profile: Partial<UserProfile>, body: { job_id?: string; message?: string }) {
    if (profile.role !== "caster") {
    ...
    try {
      ...
      throw new Error(`Application submission failed: ${getErrorMessage(e)}`);
    }
  }
  ...
  static async adminOverrideStatus(applicationId: string, status: string) {
    try {
      ...
      throw new Error(`Status override failed: ${getErrorMessage(e)}`);
    }
  }
}
