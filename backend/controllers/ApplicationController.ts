...
export class ApplicationController {
  static async getApplications(req: Request) {
    try {
      ...
      if (msg.includes("Invalid role")) return NextResponse.json({ error: msg }, { status: 403 });
  ...
  static async createApplication(req: Request) {
    try {
      ...
      const msg = getErrorMessage(e);
      if (msg.includes("already applied")) return NextResponse.json({ error: msg }, { status: 400 });
      return NextResponse.json({ error: msg }, { status: 500 });
    }
  }
